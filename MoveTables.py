...
#Pre-init rook and bishop attacks
BISHOP_ATTACKS = [0]*512
ROOK_ATTACKS = [0]*4096

for mask in BISHOP_ATTACK_MASK:
    b = [0]*512
    n = pop_count(mask)
    
    for i in range(1<<n):
         b[i] = index_to_uint64(i,n,mask)
         BISHOP_ATTACKS[i] = batt(sq,b[i])

for mask in ROOK_ATTACK_MASK:
    b = [0]*4096
    n = pop_count(mask)
    
    for i in range(1<<n):
         b[i] = index_to_uint64(i,n,mask)
         ROOK_ATTACKS[i] = ratt(sq,b[i])
